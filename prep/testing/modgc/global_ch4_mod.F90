...
!
    ...
    ! =================================================================
    ! Total emissions: emissions - soil sink
    ! =================================================================
    State_Chm%CH4_EMIS(:,:,1) = State_Chm%CH4_EMIS(:,:,2) - State_Chm%CH4_EMIS(:,:,3)

    IF ( Input_Opt%Verbose ) THEN
       WRITE(*,*) 'CH4_EMIS (kg/m2/s):'
       WRITE(*,*) 'Total        : ', SUM(State_Chm%CH4_EMIS(:,:,1))
       WRITE(*,*) 'Emissions    : ', SUM(State_Chm%CH4_EMIS(:,:,2))
       WRITE(*,*) 'Sinks        : ', SUM(State_Chm%CH4_EMIS(:,:,3))
    ENDIF
    ...
    IF ( Input_Opt%DoAnalyticalInv  .or. &
       ...
       ! emissions array
       State_Chm%CH4_EMIS(:,:,1) = State_Chm%CH4_EMIS(:,:,1) + State_Chm%CH4_EMIS(:,:,3)
