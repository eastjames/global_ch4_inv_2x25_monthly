...
do_month () {
    ...
    # config for setup only
    sed -i -e "s|SetupTemplateRundir: false|SetupTemplateRundir: true|g" config.yml
    sed -i -e "s|SetupSpinupRun: false|SetupSpinupRun: true|g" config.yml
    sed -i -e "s|SetupJacobianRuns: false|SetupJacobianRuns: true|g" config.yml
    sed -i -e "s|RunSetup: false|RunSetup: true|g" config.yml
    sed -i -e "s|DoSpinup: false|DoSpinup: true|g" config.yml
    sed -i -e "s|DoJacobian: true|DoJacobian: false|g" config.yml

    # edit run dir
    sed -i -e "s|#SBATCH -c 4|#SBATCH -c 32|g" run_imi.sh
