...
echo "sbatch --array=${JOBS:1}%4000 --mem $JacobianMemory -c $JacobianCPUs -t $RequestedTime -p $SchedulerPartition -W run_jacobian_simulations.sh" >> ${BaseProjDir}/cases/${RunDir}/rerun_imi_output.log


sbatch --array=${JOBS:1}%4000 --mem $JacobianMemory \
-c $JacobianCPUs \
-t $RequestedTime \
